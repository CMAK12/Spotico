...
export class AuthService {
  private apiUrl = 'http://localhost:5032/api/authorization'; // URL to bakcend API
  ...
  private isAuthenticate: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(this.isAuthenticated());
  ...
  login(data: LoginDTO): Observable<Token> {
    ...
    return this.http.post<Token>(this.apiUrl, data).pipe(
      tap({
        next: (token) => {
